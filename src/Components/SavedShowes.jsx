...
const SavedShowes = () => {
  ...
  return (
    <div>
      <h2 className='text-white font-bold md:text-xl p-4'>Saved show</h2>
      <div className='relative flex items-center group'>
          ...
          { movies &&
            movies.map((item, id) => (
              <div className='w-[160px] sm:w-[200px] md:w-[240px] lg:w-[280px] inline-block cursor-pointer relative p-2' key={id}>
                <img className='w-full h-auto block' src={`https://image.tmdb.org/t/p/w500/${item?.img}`} alt='' />
        ...
        <MdChevronRight
          onClick={slideRight}
          className='bg-white absolute  rounded-full right-0 opacity-50 hover:opacity-100 cursor-pointer z-10 hidden group-hover:block'
          size={40}
        />
      </div>
    </div>
